...
class NB_model():
    ...
    def fit(self, train_data, train_label, vocab, if_use_smooth=True):
        ...
        prob_ij = train_data.groupby(['classIdx','wordIdx'])
        prob_j = train_data.groupby(['classIdx'])
        pr = (prob_ij['count'].sum() + float(1)) / (prob_j['count'].sum() + self.num_vocab) 

        #Unstack series
        pr = pr.unstack()

        for classIdx in range(1,self.num_classes+1):
        	pr.loc[classIdx,:] = pr.loc[classIdx,:].fillna(float(1)/(prob_j['count'].sum()[classIdx] + self.num_vocab))
        ...
        for docIdx in range(1, len(new_dict)+1):
            ...
            for classIdx in range(1,self.num_classes+1):
                ...
                for wordIdx in new_dict[docIdx]:
                	try:
	                	prob = self.Pr_dict[wordIdx][classIdx]
	                	mult = new_dict[docIdx][wordIdx]
	                	score_dict[classIdx]+=mult*np.log(prob)
	                except:
	                	score_dict[classIdx] += 0
...
# pred = np.array(predict_test_labels)
# i = 0
# while i < len(real):
# 	if(real[i] != pred[i]):
# 		break
# 	i+=1
# print(i)
# print("real test label: {}".format(test_label[i]))
# print("predicted test label: {}".format(predict_test_labels[i]))
