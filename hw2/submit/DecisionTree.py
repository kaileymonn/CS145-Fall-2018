...
def compute_info_gain(data, att_name, y_name):
    ...
    for i, val in enumerate(vals):
        ...
        filtered_data = data.loc[data[att_name] == val]
        partial_entropy = compute_entropy(filtered_data[y_name])
        conditional_entropy += prob * partial_entropy
...
class DecisionTree(object):
    ...
    def make_tree(self, data, parent_node_class = None):
        ...
        if len(np.unique(data[self.class_name])) <= 1:
            leaf_value = -1
            ########## Please Fill Missing Lines Here ##########
            leaf_value = data[self.class_name].values[0]
            return leaf_value
