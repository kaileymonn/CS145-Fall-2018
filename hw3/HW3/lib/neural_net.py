...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #   You may simply use np.maximum for implementing ReLU.
        # ================================================================ #
        ...
        def MSE_loss(y_pred, y_target):
            ...
            # Convert y_target to 2d one-hot array
            target = np.zeros((y_target.size, y_target.max()+1))
            target[np.arange(y_target.size),y_target] = 1.0
            
            # Compute MSE
            diff = y_pred - target
            squared_diff = diff*diff
            loss = np.sum(np.sum(squared_diff, axis=1)) / float(2*N)
                                       
            # Compute gradient
            dy_pred = -1 *(target - y_pred) / float(N)            
