...
class DataPoints:
    ...
    def getMean(clusters, mean):
        ...
        for i in range(len(mean)):
            points = np.array([[point.x, point.y] for point in clusters[i]]) 
            mean[i] = np.mean(points, axis=0).tolist()
    ...
    def getStdDeviation(clusters, mean, stddev):
        ...
        for i in range(len(stddev)):
            points = np.array([[point.x, point.y] for point in clusters[i]]) 
            stddev[i] = np.std(points, axis=0).tolist()
