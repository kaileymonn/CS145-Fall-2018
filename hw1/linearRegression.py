...
def getBeta(train_x, train_y):   
    ...
    XtY = np.matmul(Xt, train_y)
    np.matmul(XtX_inverse, XtY, out=beta)
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
            ########## Please Fill Missing Lines Here ##########
            Xt = np.transpose(train_x[i])
            XtB = np.dot(Xt, beta)
            ith_deriv = np.multiply(train_x[i], XtB - train_y[i])
            np.add(deriv, ith_deriv, out=deriv)
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            ########## Please Fill Missing Lines Here ##########
            Xt = np.transpose(train_x[idx])
            XtB = np.dot(Xt, beta)
            delta = np.multiply(lr * np.subtract(train_y[idx], XtB), train_x[idx])
            np.add(beta, delta, out=beta)
