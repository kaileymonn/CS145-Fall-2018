...
def compute_avglogL(X,y,beta):
    ...
    for i in range(n):
        Xt = np.transpose(X[i])
        XtB = np.dot(Xt, beta)
        yXtB = np.multiply(y[i], XtB)
        currL = yXtB - np.logaddexp(eps, XtB)
        totalLogL = np.add(totalLogL, currL)

    avglogL = -1 * totalLogL / n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        deriv = np.zeros(p)
        # Compute deriv element-wise
        for j in range(p):
            for i in range(n):
                xij = train_x[i][j]
                yi = train_y[i]
                Btxi = np.dot(np.transpose(beta), train_x[i])
                p_xi_B = stableSigmoid(Btxi)
                deriv[j] += xij * (yi - p_xi_B)
        ...
        for row in range(p):
            for col in range(p):
                for i in range(n):
                    ...
                    xij = train_x[i][row]
                    xin = train_x[i][col]
                    hessian[row][col] -= xij * xin * p_xi_B * (1 - p_xi_B)
...
def getBeta_RegularizedBatchGradient(train_x, train_y, lr, num_iter, verbose):
    beta = np.random.rand(train_x.shape[1])

    n = train_x.shape[0] #total of data points
    p = train_x.shape[1] #total number of attributes

    regularization_strength = 1 # new hyperparameter, set to 1 for simplicity
    ...
    for iter in range(0, num_iter):
        ...
        # Compute regularization bias term
        bias_term = 0
        for j in range(p):
            bias_term += beta[j] * 2
        bias_term = bias_term * regularization_strength

        np.divide(deriv, n, out=deriv)
        np.add(deriv, bias_term / n, out=deriv)
...
class LogisticRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            beta = getBeta_RegularizedBatchGradient(newTrain_x, self.train_y.values, self.lr, self.num_iter, self.verbose)
            print('Beta: ', beta)
